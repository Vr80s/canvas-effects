<template>
  <div>
    <canvas ref="firework-canvas"></canvas>
  </div>
</template>
...
class Firework {
  ...
  update(index) {
    ...
    // 计算出移动后的距离
    this.distanceTraveled = calcPointsDistance(
      this.sx,
      this.sy,
      this.x + vx,
      this.y + vy
...
function render() {
  ...
  // 函数循环80次自动发射8支烟花
  if (bangNum> 0 && timerTick == Math.round(timerTotal * 0.3)) {
    for (let i = 0; i < bangNum; i++) {
      fireworks.push(
        new Firework(
          canvasWidth / 2,
          canvasHeight,
  ...
  if (bangNum> 0 && timerTick == Math.round(timerTotal * 0.6)) {
    for (let i = 0; i < bangNum; i++) {
      fireworks.push(
        new Firework(
          canvasWidth / 2,
  ...
  if (bangNum> 0 && timerTick >= timerTotal) {
    for (let i = 0; i < bangNum; i++) {
        fireworks.push(
          new Firework(
            canvasWidth / 2,
...
export default {
  ...
  mounted() {
    const canvas = this.$refs["firework-canvas"];
    // canvas全屏
